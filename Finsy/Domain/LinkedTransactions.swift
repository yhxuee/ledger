...
enum TransactionSemantics {
    ...
    static func refundValueInParentCurrency(_ parent: LedgerTransaction, in state: LedgerState, index: LedgerIndex? = nil) -> Double {
        guard parent.exchangeRateAtTransaction > 0 else { return 0 }
        let refundChildren = children(of: parent, in: state, index: index).filter { $0.linkedTransactionKind == .refundIncome }
    ...
    static func children(of parent: LedgerTransaction, in state: LedgerState, index: LedgerIndex? = nil) -> [LedgerTransaction] {
        if let index {
            return index.childrenByParentID[parent.id] ?? []
        }
    ...
    static func statusPresentation(for parent: LedgerTransaction, in state: LedgerState, now: Date = .now, index: LedgerIndex? = nil) -> GroupStatusPresentation? {
        guard let mode = parent.groupMode else { return nil }
        let groupChildren = children(of: parent, in: state, index: index)
        ...
        case .refund:
            let refundVal = refundValueInParentCurrency(parent, in: state, index: index)
            if refundVal <= 0.001 { return .refundPartial }
            return refundVal < (parent.amount - 0.005) ? .refundPartial : .refundComplete

        case .combinedPayment:
            return combinedPaymentHasActiveRefund(parent, in: state, index: index) ? .refundComplete : nil
        }
    }

    static func combinedPaymentHasActiveRefund(_ parent: LedgerTransaction, in state: LedgerState, index: LedgerIndex? = nil) -> Bool {
        if let index {
            return (index.childrenByParentID[parent.id] ?? []).contains {
                $0.linkedTransactionKind == .combinedPaymentRefund && $0.deletedAt == nil
            }
        }
    ...
    static func combinedPaymentActiveItems(_ parent: LedgerTransaction, in state: LedgerState, index: LedgerIndex? = nil) -> [LedgerTransaction] {
        if let index {
            return (index.childrenByParentID[parent.id] ?? []).filter {
                $0.linkedTransactionKind == .combinedPaymentItem && $0.deletedAt == nil
            }
        }
        return state.transactions.filter {
            $0.parentTransactionID == parent.id &&
    ...
    static func combinedPaymentIsRefundable(_ parent: LedgerTransaction, in state: LedgerState, index: LedgerIndex? = nil) -> Bool {
        guard parent.groupMode == .combinedPayment, parent.deletedAt == nil else { return false }
        guard !combinedPaymentHasActiveRefund(parent, in: state, index: index) else { return false }
        return combinedPaymentActiveItems(parent, in: state, index: index).count >= 2
    }

    /// Sum of posted installment postings for refunding an installment parent.
    static func refundableInstallmentAmount(_ parent: LedgerTransaction, in state: LedgerState, now: Date = .now, index: LedgerIndex? = nil) -> Double {
        let installments = children(of: parent, in: state, index: index).filter { $0.linkedTransactionKind == .installment && $0.isCompleted(asOf: now) }
