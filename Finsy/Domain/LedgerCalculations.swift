...
enum LedgerCalculations {
    ...
    static func pocketBalance(_ currency: CurrencyCode, for account: LedgerAccount, in state: LedgerState, index: LedgerIndex? = nil) -> Double {
        pocketBalances(for: account, in: state, index: index).first(where: { $0.currency == currency })?.balance ?? 0
    }

    /// Every pocket of an account, in declaration order.
    /// Scans only transactions affecting this account, computing all pocket balances in a single pass.
    static func pocketBalances(for account: LedgerAccount, in state: LedgerState, index: LedgerIndex? = nil) -> [(currency: CurrencyCode, balance: Double)] {
        let pockets = account.normalizedPockets
        guard !pockets.isEmpty else { return [] }

        var balances: [CurrencyCode: Double] = [:]
        for pocket in pockets {
            balances[pocket.currency] = pocket.openingBalance
        }

        let relevantTransactions = index?.transactionsByAccountID[account.id] ?? activeTransactions(state).filter {
            $0.accountID == account.id || ($0.type == .transfer && $0.destinationAccountID == account.id)
        }

        for transaction in relevantTransactions {
            guard TransactionSemantics.posts(transaction) else { continue }
            if transaction.accountID == account.id {
                let pocket = sourcePocket(transaction, for: account)
                if balances[pocket] != nil {
                    let posted = sourcePosting(transaction, for: account, in: state)
                    switch transaction.type {
                    case .expense, .transfer: balances[pocket]! -= posted
                    case .income: balances[pocket]! += posted
                    }
                }
            }
            if transaction.type == .transfer, transaction.destinationAccountID == account.id {
                let pocket = destinationPocket(transaction, for: account)
                if balances[pocket] != nil {
                    balances[pocket]! += destinationPosting(transaction, for: account, in: state)
                }
            }
        }

        return pockets.map { ($0.currency, balances[$0.currency] ?? 0) }
    }
    ...
    static func balance(for account: LedgerAccount, in state: LedgerState, index: LedgerIndex? = nil) -> Double {
        if account.type == .stocks, let stock = account.stockMetadata { return stock.value }
        let pBalances = pocketBalances(for: account, in: state, index: index)
        return pBalances.reduce(0) { total, pocket in
            total + convert(pocket.balance, from: pocket.currency, to: account.currency, rates: state.settings.rates)
        }
    }
    ...
    static func accountViews(_ state: LedgerState, index: LedgerIndex? = nil) -> [AccountViewModel] {
        let accounts = index?.activeAccounts ?? activeAccounts(state)
        return accounts.map { .init(account: $0, balance: balance(for: $0, in: state, index: index)) }
    }

    static func portfolioAccounts(_ state: LedgerState, index: LedgerIndex? = nil) -> [LedgerAccount] {
        let accounts = index?.activeAccounts ?? activeAccounts(state)
        return accounts.filter { !$0.effectiveIsFrozen }
    }

    static func portfolioBalance(_ state: LedgerState, target: CurrencyCode? = nil, index: LedgerIndex? = nil, accountViews views: [AccountViewModel]? = nil) -> Double {
        portfolioSummary(state, target: target, index: index, accountViews: views).netWorth
    }

    static func portfolioSummary(_ state: LedgerState, target: CurrencyCode? = nil, index: LedgerIndex? = nil, accountViews views: [AccountViewModel]? = nil) -> (netWorth: Double, assets: Double, liabilities: Double) {
        let currency = target ?? state.settings.baseCurrency
        let items = views ?? accountViews(state, index: index)
        return items
