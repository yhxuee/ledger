...
enum LedgerPresentation {
    ...
    static func entries(
        ...
        state: LedgerState,
        collapsePurchases: Bool = true,
        now: Date = .now,
        index: LedgerIndex? = nil
    ) -> [LedgerPresentationEntry] {
        let all = index?.transactionsByID ?? Dictionary(uniqueKeysWithValues: state.transactions.filter { $0.deletedAt == nil }.map { ($0.id, $0) })
        let sessions = index?.purchaseSessionsByID ?? Dictionary(uniqueKeysWithValues: (state.purchaseSessions ?? []).map { ($0.id, $0) })
        let purchaseChildren = index?.purchaseTransactionsBySessionID ?? Dictionary(grouping: all.values.filter {
