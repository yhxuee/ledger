...
public enum StatementPDFGenerator {
    ...
    private static func drawAccountSummaryTable(summaries: [Any], yOffset: inout CGFloat) {
        // Headers
        ...
        for item in summaries {
            guard let mirror = Mirror(reflecting: item).children.reduce(into: [String: Any](), { $0[$1.label ?? ""] = $1.value }) as [String: Any]? else { continue }
            guard let acc = mirror["account"] as? LedgerAccount,
                  let open = mirror["openingBalance"] as? Double,
                  let inf = mirror["totalInflow"] as? Double,
                  let outf = mirror["totalOutflow"] as? Double,
                  let close = mirror["closingBalance"] as? Double else { continue }
    ...
    private static func drawTaxRow(item: Any, in state: LedgerState, yOffset: inout CGFloat) {
        guard let mirror = Mirror(reflecting: item).children.reduce(into: [String: Any](), { $0[$1.label ?? ""] = $1.value }) as [String: Any]? else { return }
        guard let t = mirror["transaction"] as? LedgerTransaction,
              let catName = mirror["categoryName"] as? String,
              let taxAmt = mirror["taxAmount"] as? Double,
              let taxBase = mirror["taxBase"] as? Double else { return }
