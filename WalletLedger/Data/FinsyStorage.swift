...
enum FinsyStorage {
    ...
    private static func migrate() throws {
        ...
        let legacy = destination.deletingLastPathComponent().appending(path: FinsyCompatibility.storageDirectory)
        ...
        let staging = destination.deletingLastPathComponent().appending(path: "finsy-migration-\(UUID().uuidString)")
        ...
        do {
            ...
            for case let file as URL in files {
                ...
                guard manager.contentsEqual(atPath: file.path, andPath: staging.appending(path: relative).path) else { throw CocoaError(.fileReadCorruptFile) }
