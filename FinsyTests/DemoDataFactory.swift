...
/// Test-only data factory providing realistic financial fixtures for unit tests, previews, and auditing.
/// This factory is separated from production runtime and is compiled exclusively into test targets.
enum DemoDataFactory {
    ...
    static func make(now: Date = .now) -> LedgerState {
        ...
        let rates = SeedData.rates
        let categories = SeedData.categories
        let localUserID = SeedData.localUserID
        ...
        func append(_ id: Int, _ type: LedgerTransactionType, _ source: LedgerAccount,
            ...
            let isExpenseOrIncome = (type == .expense || type == .income)
            let categoryValue = isExpenseOrIncome ? categories.first { $0.id == category } : nil
    ...
    static func makeWithSingleAccount(now: Date = .now) -> LedgerState {
        var state = SeedData.makeProductionEmpty()
        let account = LedgerAccount(
            id: UUID(), userID: SeedData.localUserID, name: "Main Account", type: .checking,
...
extension SeedData {
    @available(*, deprecated, message: "Use DemoDataFactory.make() for test fixtures")
    static func make(now: Date = .now) -> LedgerState {
        DemoDataFactory.make(now: now)
    }
}
